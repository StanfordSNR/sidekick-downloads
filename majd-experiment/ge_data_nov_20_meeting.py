...
DATA_SIZE = 20*125000*10 * 2 # doubled data size to be 50MB
...
def plot_ge_vs_throughput_bars(raw_data, title=None, pdf=None, style=False):
    ...
    # Extract delay1 from first network setting and data size
    delay1 = exp.get_network_setting(cat_order_verbose[0]).get('delay1')
    data_size_str_val = data_size_str(data.data_sizes[0]) + "B"  # Add "B" to make it "50MB" instead of "50M"
    if title is None:
        title = f"Goodput under GE — Data={data_size_str_val}, delay1={delay1}ms"
    else:
        title = f"Goodput under GE — Data={data_size_str_val}, delay1={delay1}ms"
    plt.title(title)
...
def plot_ge_vs_throughput_bars_faceted(raw_data, title=None, pdf=None, style=False):
    ...
    # Extract delay1 from first network setting and data size
    delay1 = exp.get_network_setting(cat_verbose[0]).get('delay1')
    data_size_str_val = data_size_str(data.data_sizes[0]) + "B"  # Add "B" to make it "50MB" instead of "50M"
    if title is None:
        title = f"Goodput under GE — Data={data_size_str_val}, delay1={delay1}ms"
    else:
...
for delay in DELAYS:
    network_settings = NETWORK_SETTINGS_GE(delay1=delay)
    loss_vs_throughput_data_ge = collect_loss_vs_metric_data(TREATMENTS(), network_settings, DATA_SIZE, n=NUM_TRIALS, execute=True)

    plot_ge_vs_throughput_bars(loss_vs_throughput_data_ge, pdf=f"figures_ge_nov_20/http_benchmark_ge_bars_{now_str}_delay_{delay}.pdf")
